...
import com.ricka.princy.bonjouraurevoir.endpoint.rest.security.readme.monitor.model.entry.*;
...
public class ReadmeEntryFactory {
        ...
        return ReadmeEntry
            .builder()
                ...
                .response(ReadmeEntryResponse.builder()
                    ...
                    .content(ReadmeEntryResponseContent.builder().mimeType(response.getContentType()).build())
                    .build()
                )
            .build();
    }
    ...
    private List<ReadmeEntryHeader> retrieveHeaders(HttpServletResponse response){
        return response
            .getHeaderNames()
            .stream()
            .map(headerName -> ReadmeEntryHeader
                .builder()
                    .name(headerName)
    ...
    private List<ReadmeEntryHeader> retrieveHeaders(HttpServletRequest request){
        ...
        while (headerNames.hasMoreElements()) {
            ...
            headers.add(ReadmeEntryHeader.builder().name(headerName).value(request.getHeader(headerName)).build());
        }
        return headers;
    }
    ...
    private List<ReadmeEntryQuery> retrieveQueries(HttpServletRequest request){
        ...
        while (parameterNames.hasMoreElements()) {
            ...
            parameters.add(ReadmeEntryQuery.builder().name(parameterName).value(request.getParameter(parameterName)).build());
        }
        return parameters;
    }
}
